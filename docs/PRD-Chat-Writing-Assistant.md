# AI Writing Assistant — Chat Tab 产品需求文档
...
当一场 Zoom 会议结束后，AI Writing Assistant 会自动从会议转录中识别后续的写作任务（例如"发一条 Sprint 更新""给利益相关者发邮件"等），并主动通过 **Zoom Team Chat** 向用户发送通知。
...
```
┌── Bot 头像 ────┬── 发送者信息栏 ──────────────────────────┐
│  [AI 图标]     │  AI Writing Assistant  [APP]  {时间戳}   │
│                ├───────────────────────────────────────────┤
│                │  ┌─ 消息卡片 ─────────────────────────┐   │
│                │  │  {标题行}                          │   │
│                │  │  {正文内容 — 因消息类别而异}        │   │
...
| **发送者信息栏** | `AI Writing Assistant`（加粗）+ `APP` 徽章 + 时间戳。位于卡片上方。 |
